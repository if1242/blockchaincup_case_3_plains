...
const TX_ISSUE_ID = 2
const TX_WALLET_ID = 4
const TX_TRANSFER_DETAIL_ID = 1
const TX_ISSUE_DETAIL_ID = 3
...
const TransferDetailTransaction = {
  protocol_version: PROTOCOL_VERSION,
  service_id: SERVICE_ID,
  message_id: TX_TRANSFER_DETAIL_ID,
  fields: [
    { name: 'from', type: Exonum.PublicKey },
    { name: 'to', type: Exonum.PublicKey },
    { name: 'name_of_type', type: Exonum.Uint8 },
    { name: 'count', type: Exonum.Uint64 },
    { name: 'seed', type: Exonum.Uint64 }
  ]
}
...
const IssueDetailTransaction = {
  protocol_version: PROTOCOL_VERSION,
  service_id: SERVICE_ID,
  message_id: TX_ISSUE_DETAIL_ID,
  fields: [
    { name: 'pub_key', type: Exonum.PublicKey },
    { name: 'name_of_type', type: Exonum.Uint8 },
    { name: 'seed', type: Exonum.Uint64 }
...
let Detail = Exonum.newType({
  fields: [
	{ name: 'name_of_type', type: Exonum.Uint8 },
	{ name: 'count', type: Exonum.Uint64 }
  ]	
})
let DetailVec = Exonum.newArray({
	type: Detail
})
const Wallet = Exonum.newType({
  fields: [
    ...
    { name: 'details', type: DetailVec },
    { name: 'develop', type: Exonum.newArray({type: Detail}) }, 
...
function getTransaction(id) {
  switch (id) {
    ...
    case TX_ISSUE_DETAIL_ID:
      return Exonum.newMessage(IssueDetailTransaction)
    case TX_TRANSFER_DETAIL_ID:
      return Exonum.newMessage(TransferDetailTransaction) 
    default:
    ...
    return axios.get(`/api/services/cryptocurrency/v1/wallets/info/${publicKey}`)
      ...
      .then(data => {
        ...
        if (typeof wallet === 'undefined') {
          throw new Error('Wallet not found1')
        }
...
module.exports = {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
      addDetails(keyPair, name_of_typeToAdd) {
        const TxIssueDetail = getTransaction(TX_ISSUE_DETAIL_ID)

        const data = {
          pub_key: keyPair.publicKey,
          //name_of_type: name_of_typeToAdd.toString(),
          name_of_type: name_of_typeToAdd,
          seed: Exonum.randomUint64()
        }

        const signature = TxIssueDetail.sign(keyPair.secretKey, data)
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////     
      transferDetails(keyPair, receiver, name_of_typeToTransfer, countToTransfer) {
        const TxTransferDetail = getTransaction(TX_TRANSFER_DETAIL_ID)

        const data = {
          from: keyPair.publicKey,
          to: receiver,
          name_of_type: name_of_typeToTransfer,
          count: countToTransfer,
          seed: Exonum.randomUint64()
        }

        const signature = TxTransferDetail.sign(keyPair.secretKey, data)
